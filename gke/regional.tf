# Resource definitions for regional cluster and node pools

# For the module to be able to offer regional or zonal cluster both kinds must  
...
# region and zone being defined in the same resource definition.
...
resource "google_container_node_pool" "regional_pool" {
  count = "${var.gcp_zone == "" ? length(var.node_pools) : 0}"

  # The region or zone in which the cluster resides
  region = "${google_container_cluster.regional_cluster.region}"
